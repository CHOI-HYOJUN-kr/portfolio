...
공급 → 컨베이어 이송 → Pick → 적재 공정을 제어한 **교육용 실물 MPS 프로젝트**입니다.

**개인 과제 · 2026년 1학기** · Mitsubishi Q Series / 2축 서보 / GX Works2 / GP-Pro EX

- **담당:** 자동·수동 시퀀스와 위치결정, HMI 화면 6개, 오류 표시와 작업 재개 기능을 구현했습니다.
- **문제 → 변경:** 8번째 적재가 실패했을 때 어느 단계에서 문제가 생겼는지 찾기 어려워, STEP과 완료 조건을 기준으로 시퀀스를 다시 구성했습니다.
- **검증 결과:** 순차·지그재그 방식으로 각각 8개를 적재했고, 중단 후에는 수량과 인덱스를 유지한 채 작업을 재개했습니다.
...
**공급과 이동 → Pick → Place → 최종 적재** 순서로 동작합니다. 핵심 구간 4개를 이어 붙인 13초·2배속 미리보기입니다.
...
**1. 어느 단계에서 멈췄는지 알 수 있는 시퀀스**

타이머 중심으로 이어 붙였던 공정을 STEP 릴레이와 완료 비트를 기준으로 나눴습니다. 현재 STEP은 D300에 표시하고, D320 오류 코드와 HMI 진단 화면에서 멈춘 단계를 파악할 수 있도록 했습니다.

**2. 간섭하지 않는 두 동작만 중첩**

STEP30에서는 공급과 POS_PICK 이동이 겹치도록 했습니다. Pick은 **공급 완료 + 컨베이어 안정화 + 서보 이동 완료** 조건이 모두 충족되면 시작합니다. [STEP별 완료 조건과 Busy Seen 수정](docs/control-design.md)

**3. Reset과 Resume 분리**

Reset과 Resume의 역할을 나눴습니다. Reset은 전체 상태를 초기화하고, Resume은 중단 당시의 상태를 유지한 채 작업을 재개합니다. 중단 후에는 완료 수량과 다음 적재 위치를 보존했습니다. HMI 수동운전으로 장비와 공작물 상태를 점검·조정한 뒤 자동운전을 이어갔습니다.

## 결과와 기술 자료

8개를 적재하고 중단 후 작업을 재개한 과정은 최종 보고서와 수행 기록에 남겼습니다. 보고서에 적힌 **12:00 → 10:25**는 수업 장비에서 기록한 시간으로, 양산 성과로 보기는 어렵습니다. [시간 기록과 측정 조건](docs/results.md)에 자세히 적었습니다.
